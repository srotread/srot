...
const Button: FC<ButtonProps> = ({
  ...
  if (type === "Primary") {
    return (
      <button
        className={`rounded bg-accent px-3 py-2 text-dark shadow-btn-accent transition-all focus:ring disabled:cursor-not-allowed ${
          theme === "Light"
      ...
      >
        {!loading ? (
          ...
          <div role="status">
            <svg
              aria-hidden="true"
              className="h-8 w-8 animate-spin fill-dark text-gray-200 dark:text-gray-200"
              viewBox="0 0 100 101"
              fill="none"
  ...
  if (type === "Secondary") {
    return (
      <button
        className={`focus:border-3 rounded border-2 px-3 py-2 transition-all ${
          theme === "Light"
